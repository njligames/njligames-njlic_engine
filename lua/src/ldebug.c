...
static void collectvalidlines(lua_State *L, Closure *f)
{
    if (noLuaClosure(f))
    ...
    else
        {
            ...
            for (i = 0; i < f->l.p->sizelineinfo;
                 i++) /* for all lines with code */
                ...
                case OP_LOADNIL:
                    {
                        int b = GETARG_B(i);
                        if (a <= reg &&
                            reg <= a + b) /* set registers from 'a' to 'a+b' */
                ...
                case OP_GETTABLE:
                    {
                        ...
                        const char *vn =
                            (op == OP_GETTABLE) /* name of indexed variable */
                                ? luaF_getlocalname(p, t + 1, pc)
                                : upvalname(p, t);
                        kname(p, pc, k, name);
                        return (vn && strcmp(vn, LUA_ENV) == 0) ? "global"
                                                                : "field";
                    }
                case OP_GETUPVAL:
                    {
...
static const char *getfuncname(lua_State *L, CallInfo *ci, const char **name)
{
    ...
    switch (GET_OPCODE(i))
        ...
        case OP_SHR:
            {
                int offset =
                    cast_int(GET_OPCODE(i)) - cast_int(OP_ADD); /* ORDER OP */
    ...
    if (mask & LUA_MASKLINE)
        {
            ...
            if (npc == 0 || /* call linehook when enter a new function, */
                ci->u.l.savedpc <=
                    L->oldpc || /* when jump back (loop), or when */
                newline !=
                    getfuncline(p, pcRel(L->oldpc, p))) /* enter a new line */
