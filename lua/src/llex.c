                ...
                case EOZ:
                    { /* error */
                        ...
                        const char *msg = luaO_pushfstring(
                            ls->L, "unfinished long %s (starting at line %d)",
                            what, line);
                    ...
                    { /* '-' or '--' (comment) */
                        ...
                        if (ls->current == '[')
                            { /* long comment? */
                                int sep = skip_sep(ls);
                                luaZ_resetbuffer(
                                    ls->buff); /* 'skip_sep' may dirty the
                                                  buffer */
                                if (sep >= 0)
                                    {
                                        read_long_string(
                                            ls, NULL,
                                            sep); /* skip long comment */
                                        luaZ_resetbuffer(
                                            ls->buff); /* previous call may
                                                          dirty the buff. */
                                        break;
                                    }
                            }
                ...
                case '[':
                    { /* long string or simply '[' */
                        ...
                        if (sep >= 0)
                            ...
                            lexerror(ls, "invalid long string delimiter",
                                     TK_STRING);
                        return '[';
                    }
                case '=':
