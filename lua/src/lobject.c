            ...
            else if (lisxdigit(cast_uchar(*s)))
                {
                    if (sigdig == 0 &&
                        *s == '0') /* non-significant digit (zero)? */
                        ...
                        e++; /* too many digits; ignore, but still count for
                                exponent */
                    if (hasdot)
        ...
        {                            /* need continuation bytes */
            ...
            do
                { /* add continuation bytes */
                    ...
                    mfb >>= 1; /* now there is one less bit available in first
                                  byte */
                }
            while (x > mfb); /* still needs continuation byte? */
            buff[UTF8BUFFSZ - n] =
                cast(char, (~mfb << 1) | x); /* add first byte */
        }
    return n;
}
    ...
    for (;;)
        {
            ...
            switch (*(e + 1))
                ...
                case 'f':
                    {
                        setfltvalue(L->top,
                                    cast_num(va_arg(argp, l_uacNumber)));
                    top2str:
                        luaD_inctop(L);
                ...
                case 'p':
                    {
                        ...
                        int l = l_sprintf(buff, sizeof(buff), "%p",
                                          va_arg(argp, void *));
                        pushstr(L, buff, l);
                        break;
                    }
                case 'U':
                    {
                        ...
                        int l =
                            luaO_utf8esc(buff, cast(long, va_arg(argp, long)));
