    ...
    for (loop = 0; loop < MAXTAGLOOP; loop++)
        {
            ...
            if (luaV_fastget(L, t, key, tm, luaH_get))
                {                         /* try fast track */
            ...
            if (oldval != NULL)
                {
                    ...
                    if ((tm = fasttm(L, hvalue(t)->metatable, TM_NEWINDEX)) ==
                            NULL &&
                        /* no metamethod; is there a previous entry in the
                           table? */
                        (oldval != luaO_nilobject ||
                         /* no previous entry; must create one. (The next test
                            is always true; we only need the assignment.) */
                         (oldval = luaH_newkey(L, hvalue(t), key), 1)))
                        {
                            /* no metamethod and (now) there is an entry with
                             * given key */
            ...
            if (temp != 0)   /* not equal? */
                ...
                { /* strings are equal up to a '\0' */
                    ...
                    if (len == lr) /* 'rs' is finished? */
                        ...
                        return -1; /* 'ls' is smaller than 'rs' ('rs' is not
                                      finished) */
    ...
    if (!l_intfitsf(i))
        {
            if (f >= -cast_num(LUA_MININTEGER))    /* -minint == maxint + 1 */
                ...
                return (i <
                        cast(lua_Integer, f)); /* compare them as integers */
    ...
    if (!l_intfitsf(i))
        {
            if (f >= -cast_num(LUA_MININTEGER)) /* -minint == maxint + 1 */
                return 1;                       /* f >= maxint + 1 > i */
                     ...
                     cast_num(LUA_MININTEGER)) /* minint <= f <= maxint ? */
                return (i <=
                        cast(lua_Integer, f)); /* compare them as integers */
            else /* f < minint <= i (or 'f' is NaN)  -->  not(i <= f) */
                return 0;
        ...
        { /* not the same variant? */
            if (ttnov(t1) != ttnov(t2) || ttnov(t1) != LUA_TNUMBER)
                ...
                { /* two numbers with different variants */
                    ...
                    return (tointeger(t1, &i1) && tointeger(t2, &i2) &&
                            i1 == i2);
                }
        }
...
lua_Integer luaV_mod(lua_State *L, lua_Integer m, lua_Integer n)
{
    if (l_castS2U(n) + 1u <= 1u)
    ...
    else
        {
            lua_Integer r = m % n;
            if (r != 0 &&
                (m ^ n) < 0) /* 'm/n' would be non-integer negative? */
        ...
        { /* is there a cached closure? */
            ...
            for (i = 0; i < nup; i++)
                { /* check whether it has right upvalues */
                    TValue *v =
                        uv[i].instack ? base + uv[i].idx : encup[uv[i].idx]->v;
                    if (c->upvals[i]->v != v)
...
void luaV_finishOp(lua_State *L)
{
    ...
    switch (op)
        ...
        case OP_CONCAT:
            {
                StkId top =
                    L->top - 1; /* top when 'luaT_trybinTM' was called */
                int b = GETARG_B(inst); /* first element to concatenate */
                int total =
                    cast_int(top - 1 - (base + b)); /* yet to concatenate */
                setobj2s(L, top - 2,
                ...
                if (total > 1)
                    { /* are there elements to concat? */
                        ...
                        luaV_concat(L,
                                    total); /* concat them (may yield again) */
                    }
...
#define gettableProtected(L, t, k, v)                                          \
    {                                                                          \
        ...
        if (luaV_fastget(L, t, k, aux, luaH_get))                              \
            ...
            Protect(luaV_finishget(L, t, k, v, aux));                          \
    }
    ...
    for (;;)
        {
            ...
            vmdispatch(GET_OPCODE(i))
            {
                ...
                vmcase(OP_NOT)
                {
                    TValue *rb = RB(i);
                    int res = l_isfalse(
                        rb); /* next assignment may change this value */
                    setbvalue(ra, res);
                    vmbreak;
                }
                vmcase(OP_LEN)
                ...
                vmcase(OP_CONCAT)
                {
                    ...
                    ra = RA(i); /* 'luaV_concat' may invoke TMs and move the
                                   stack */
                    rb = base + b;
                    setobjs2s(L, ra, rb);
                ...
                vmcase(OP_LE)
                {
                    Protect(if (luaV_lessequal(L, RKB(i), RKC(i)) !=
                                GETARG_A(i)) ci->u.l.savedpc++;
                            else donextjump(ci);) vmbreak;
                }
                vmcase(OP_TEST)
                {
                ...
                vmcase(OP_CALL)
                {
                    ...
                    if (luaD_precall(L, ra, nresults))
                        ...
                        { /* Lua function */
                            ci = L->ci;
                            goto newframe; /* restart luaV_execute over new Lua
                                              function */
                        }
                    vmbreak;
                }
                vmcase(OP_TAILCALL)
                {
                    ...
                    if (luaD_precall(L, ra, LUA_MULTRET))
                    ...
                    else
                        {
                            ...
                            StkId lim =
                                nci->u.l.base + getproto(nfunc)->numparams;
                            int aux;
                            ...
                            oci->u.l.base = ofunc + (nci->u.l.base -
                                                     nfunc); /* correct base */
                            oci->top = L->top =
                                ofunc + (L->top - nfunc); /* correct top */
                            oci->u.l.savedpc = nci->u.l.savedpc;
                            oci->callstatus |=
                                CIST_TAIL;    /* function was tail called */
                            ci = L->ci = oci; /* remove new frame */
                            lua_assert(L->top ==
                                       oci->u.l.base +
                                           getproto(ofunc)->maxstacksize);
                            goto newframe; /* restart luaV_execute over new Lua
                                              function */
                        }
                    vmbreak;
                ...
                vmcase(OP_RETURN)
                {
                    ...
                    if (cl->p->sizep > 0)
                        luaF_close(L, base);
                    b = luaD_poscall(L, ci, ra,
                                     (b != 0 ? b - 1 : cast_int(L->top - ra)));
                    if (ci->callstatus &
                        CIST_FRESH) /* local 'ci' still from callee */
                        ...
                        { /* invocation via reentry: continue execution */
                            ...
                            if (b)
                                L->top = ci->top;
                            lua_assert(isLua(ci));
                            lua_assert(GET_OPCODE(*((ci)->u.l.savedpc - 1)) ==
                                       OP_CALL);
                            goto newframe; /* restart luaV_execute over new Lua
                                              function */
                        }
                ...
                vmcase(OP_FORLOOP)
                {
                    if (ttisinteger(ra))
                        { /* integer loop? */
                            ...
                            lua_Integer idx = intop(+, ivalue(ra),
                                                    step); /* increment index */
                            lua_Integer limit = ivalue(ra + 1);
                            if ((0 < step) ? (idx <= limit) : (limit <= idx))
                                {
                                    ci->u.l.savedpc +=
                                        GETARG_sBx(i); /* jump back */
                                    chgivalue(
                                        ra, idx); /* update internal index... */
                                    setivalue(ra + 3,
                                              idx); /* ...and external index */
                                }
                        }
                    else
                        { /* floating loop */
                            lua_Number step = fltvalue(ra + 2);
                            lua_Number idx = luai_numadd(L, fltvalue(ra),
                                                         step); /* inc. index */
                    ...
                    if (ttisinteger(init) && ttisinteger(pstep) &&
                        ...
                        { /* try making all values floats */
                            ...
                            if (!tonumber(init, &ninit))
                                luaG_runerror(
                                    L, "'for' initial value must be a number");
                ...
                vmcase(OP_SETLIST)
                {
                    ...
                    if (c == 0)
                        {
                            lua_assert(GET_OPCODE(*ci->u.l.savedpc) ==
                                       OP_EXTRAARG);
                    ...
                    if (last > h->sizearray) /* needs more space? */
                        luaH_resizearray(L, h,
                                         last); /* preallocate it at once */
                    for (; n > 0; n--)
                        {
                            TValue *val = ra + n;
                    ...
                    L->top = ci->top; /* correct top (in case of previous open
                                         call) */
                    vmbreak;
                }
                vmcase(OP_CLOSURE)
                {
                    ...
                    LClosure *ncl =
                        getcached(p, cl->upvals, base); /* cached closure */
                    if (ncl == NULL)                    /* no match? */
                        pushclosure(L, p, cl->upvals, base,
                                    ra); /* create a new one */
                    else
