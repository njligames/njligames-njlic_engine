    ...
    if (!testnext(ls, what))
        {
            if (where == ls->linenumber)
            ...
            else
                {
                    luaX_syntaxerror(
                        ls, luaO_pushfstring(
                                ls->L, "%s expected (to close %s at line %d)",
                    ...
                    if (idx < 0)
                        { /* not found? */
                            if (singlevaraux(fs->prev, n, var, 0) ==
                                VVOID)        /* try upper levels */
                                return VVOID; /* not found; is a global */
                            /* else was LOCAL or UPVAL */
                            idx = newupvalue(fs, n,
                                             var); /* will be a new upvalue */
                        }
                    ...
                    if (lh->v.u.ind.vt == v->k && lh->v.u.ind.t == v->u.info)
                        {
                            ...
                            lh->v.u.ind.t = extra; /* previous assignment will
                                                      use safe copy */
                        }
                    /* index is the local being assigned? (index cannot be
                     * upvalue) */
                    if (v->k == VLOCAL && lh->v.u.ind.idx == v->u.info)
                        {
                            conflict = 1;
                            lh->v.u.ind.idx = extra; /* previous assignment will
                                                        use safe copy */
                        }
                }
        }
            ...
            if (nexps != nvars)
                {
                    ...
                    if (nexps > nvars)
                        ls->fs->freereg -=
                            nexps - nvars; /* remove extra values */
                }
            else
                {
    ...
    for (i = fs->bl->firstlabel; i < ll->n; i++)
        {
            if (eqstr(label, ll->arr[i].name))
                {
                    const char *msg = luaO_pushfstring(
                        fs->ls->L, "label '%s' already defined on line %d",
...
static void test_then_block(LexState *ls, int *escapelist)
{
    ...
    if (ls->t.token == TK_GOTO || ls->t.token == TK_BREAK)
        {
            luaK_goiffalse(ls->fs,
                           &v); /* will jump to label if condition is true */
            ...
            if (hasmultret(e.k))
                {
                    ...
                    if (e.k == VCALL && nret == 1)
                        { /* tail call? */
                            ...
                            lua_assert(GETARG_A(getcode(fs, &e)) ==
                                       fs->nactvar);
                        }
                    first = fs->nactvar;
            ...
            else
                {
                    if (nret == 1) /* only one single value? */
                        ...
                        {
                            luaK_exp2nextreg(
                                fs, &e); /* values must go to the stack */
