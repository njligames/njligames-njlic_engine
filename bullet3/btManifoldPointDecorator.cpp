...
#define FORMATSTRING                                                           \
    ...
    "\"Position On A\":%s, \"Position On B\":%s, \"Applied "                   \
    "Impulse\":\"%f\"}]}"

btManifoldPointDecorator::btManifoldPointDecorator(btManifoldPoint *p)
    : m_btManifoldPoint(p)
{
}
...
btManifoldPointDecorator::operator std::string() const
{
    ...
    std::string temp(
        string_format(FORMATSTRING, m_btManifoldPoint->getDistance(),
                      m_btManifoldPoint->getLifeTime(), pa.c_str(), pb.c_str(),
