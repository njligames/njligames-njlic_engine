...
btQuaternionDecorator::btQuaternionDecorator(btQuaternion *p) : m_btQuaternion(p) {}
...
btQuaternionDecorator::operator std::string() const {
    ...
    std::string temp(string_format(FORMATSTRING,
                                   m_btQuaternion->getAngle(),
                                   axis.c_str()));
    return temp;
}

btQuaternionDecorator::operator glm::quat() const {
    return glm::quat(m_btQuaternion->w(), m_btQuaternion->x(), m_btQuaternion->y(), m_btQuaternion->z());
}
