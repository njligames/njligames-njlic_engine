...
namespace njli
{
    Scene::Scene()
        : AbstractFactoryObject(this),
          ...
          m_ActiveParticleEmitters(
              new btAlignedObjectArray<ParticleEmitter *>()),
    ...
    //              geometry->applyShape(node, physicsBody->getPhysicsShape());
    //
    //            geometry->setTransform(geometryIndex,
    //            node->getWorldTransform());
    //
    ...
    ////              const float theta = 2.0f * M_PI * (static_cast<float>(x) /
    /// static_cast<float>(width)); // [0, 2 pi]
    //              const float theta = 2.0f * M_PI * 0.5f; // [0, 2 pi]
    //              const float theta_offset = theta + ( is_left ? 0.0f : M_PI
    //              );
    ////              const float phi = (fmodf( 2.0f * ( 0.5f * screen_y + 0.5f
    ///) , 1.0f ) - 0.5f ) * M_PI;
    //              const float phi = (fmodf( 2.0f * ( 0.5f * 0.5f + 0.5f )
    //              , 1.0f ) - 0.5f ) * M_PI;
    //
    //
    //
    ...
    ////
    ////
    //              btVector3 origin(0.5f * ipd * (-cosf(theta_offset)), 0.0f,
    //              0.5f * ipd * (sinf(theta_offset))); btVector3
    ...
    void Scene::addActiveClock(Clock *clock)
    {
        ...
        if ((*m_ActiveClocks).size() ==
            (*m_ActiveClocks).findLinearSearch(clock))
