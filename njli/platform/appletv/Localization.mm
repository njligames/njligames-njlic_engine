...
std::string LOCALIZED_TEXT(const char *key, const char *default_value,
                           const char *comment)
{
  NSString *_key = [[NSString alloc] initWithCString:key
  ...
  NSString *value =
      [NSBundle.mainBundle localizedStringForKey:(_key)
                                           value:(_val)table:(_tbl)];
...
std::string LOCALIZED_LANGUAGE()
{
  ...
  const char *_value =
      [preferredLang cStringUsingEncoding:NSUTF8StringEncoding];
  return std::string(_value);
}
