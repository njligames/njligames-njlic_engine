...
namespace njli
{
    ...
    bool NJLIGameEngine::isMACOSX()
    {
        return platformID() == PlatformID_MacOSx;
    }
                                ...
                                int orientation)
    {
        if (njli::World::hasInstance())
            njli::World::getInstance()->resize(x, y, width, height,
                                               orientation);
    }
    ...
    bool NJLIGameEngine::start(int argc, char **argv)
    {
        ...
        //    bool ret = false;
        //    if
        //    (World::getInstance()->getWorldLuaVirtualMachine()->compileString(
        //            source.c_str()))
        //      {
        //        ret =
    ...
    void NJLIGameEngine::mouse(int button, int eventType, float x, float y,
                               int clicks)
    {
        njli::World::getInstance()->getWorldInput()->handleMouse(
            button, eventType, x, y, clicks);
    }
                               ...
                               bool withGui)
    {
        njli::World::getInstance()->getWorldInput()->handleKeyUp(
            keycodeName, withCapsLock, withControl, withShift, withAlt,
            withGui);
    }
