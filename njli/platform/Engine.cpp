...
#undef BUTTON_CASE
    ...
    static void SDLTest_PrintEvent(SDL_Event *event)
    {
        if (event->type == SDL_FINGERMOTION)
            {
                ...
                //        SDL_Log("SDL EVENT: Finger: %s touch=%ld, finger=%ld,
                //        x=%f, y=%f, dx=%f, dy=%f, pressure=%f",
                //                (event->type == SDL_FINGERDOWN) ? "down" :
                //                "up", (long) event->tfinger.touchId, (long)
                //                event->tfinger.fingerId, event->tfinger.x,
        ...
        switch (event->type)
            ...
            case SDL_JOYDEVICEMOTION:
                {
                    SDL_Log("Joystick device %d motion. [%f, %f, %f, %f, %f, "
                            ...
                            event->jmotion.m12, event->jmotion.m13,
                            event->jmotion.m21, event->jmotion.m22,
                            event->jmotion.m23, event->jmotion.m31,
            ...
            case SDL_KEYDOWN:
                SDL_Log(
                    "SDL EVENT: Keyboard: key pressed  in window %d: scancode "
            ...
            case SDL_KEYUP:
                SDL_Log(
                    "SDL EVENT: Keyboard: key released in window %d: scancode "
                    "0x%08X = %s, keycode 0x%08X = %s",
                    event->key.windowID, event->key.keysym.scancode,
                    SDL_GetScancodeName(event->key.keysym.scancode),
                    event->key.keysym.sym,
            ...
            case SDL_MOUSEBUTTONDOWN:
                SDL_Log("SDL EVENT: Mouse: button %d pressed at %d,%d with "
                        "click count "
                        "%d in window %d",
            ...
            case SDL_MOUSEBUTTONUP:
                SDL_Log(
                    "SDL EVENT: Mouse: button %d released at %d,%d with click "
                    "count %d in window %d",
                    event->button.button, event->button.x, event->button.y,
                    event->button.clicks, event->button.windowID);
                break;
            ...
            case SDL_JOYDEVICEADDED:
                {
                    ...
                    if (joystick == NULL)
                        ...
                        {
                            gGameJoystickMap.insert(JoystickPair(
            ...
            case SDL_FINGERUP:
                SDL_Log(
                    "SDL EVENT: Finger: %s touch=%ld, finger=%ld, x=%f, y=%f, "
    ...
    static int EventFilter(void *userdata, SDL_Event *event)
    {
        ...
        //      case SDL_JOYDEVICEMOTION:
        //        {
        //          //              SDL_Log("Joystick device %d motion. [%f, %f,
        //          %f, %f,
        //          //              %f, %f, %f, %f, %f]\n", (int)
        //          event->jmotion.which,
        ...
        //          //              event->jmotion.m33); SDL_Log("Yaw, Pitch,
        //          Roll
        //          (%f,
        //          //              %f, %f)\n", event->jmotion.yaw,
        //          event->jmotion.pitch,
...
//     (defined(__ANDROID__) && __ANDROID__))
        ...
        while (SDL_PollEvent(&event))
            {
                ...
                switch (event.type)
                    ...
                    case SDL_KEYDOWN:
                        {
                            ...
                            switch (event.key.keysym.sym)
                                ...
                                case SDLK_c:
                                    ...
                                    if (withAlt)
                                        {
                                            /* Alt-C toggle a render clip
                                             * rectangle */
                                            int w, h;
                                            if (gRenderer)
                                                {
                                                    SDL_Rect clip;
                                                    SDL_GetWindowSize(gWindow,
                                                                      &w, &h);
                    ...
                    case SDL_MOUSEWHEEL:
                        SDL_Log("SDL EVENT: Mouse: wheel scrolled %d in x and "
                                "%d in y "
                                "(reversed: %d) in window %d",
                                event.wheel.x, event.wheel.y,
                                event.wheel.direction, event.wheel.windowID);
    ...
    static void SDLTest_PrintRenderer(SDL_RendererInfo *info)
    {
        ...
        fprintf(stderr,
                "    Texture formats (%d): ", info->num_texture_formats);
        ...
        if (argv[1])
            {
                ...
                //                  if (joystick == NULL) {
                //                      if ( !reportederror ) {
                //                          SDL_Log("Couldn't open joystick %d:
                //                          %s\n", device, SDL_GetError());
                //                          keepGoing = SDL_FALSE; reportederror
                //                          = SDL_TRUE;
                //                      }
                //                  } else {
...
//      SDL_GL_CONTEXT_PROFILE_CORE);
        ...
        //    int w, h;
        //    SDL_GetWindowSize(gWindow, &w, &h);
        //    SDL_Log("SDL_GetWindowSize #%d: current display mode is %dx%dpx",
        //    0, w, h);
        ...
        //    SDL_GL_GetDrawableSize(gWindow, &w, &h);
        //    SDL_Log("SDL_GL_GetDrawableSize #%d: current display mode is
        //    %dx%dpx", 0, w, h);
...
//      SDL_GL_GetDrawableSize(gWindow, &w, &h);
        ...
        /* In OpenGL, point sizes are always in pixels. We don't want them
         * looking tiny on a retina screen. */
...
#if defined(__EMSCRIPTEN__)
        //           int fullScreen = 0;
        //           emscripten_get_canvas_size(&gDisplayMode.w,
        //           &gDisplayMode.h, &fullScreen);
