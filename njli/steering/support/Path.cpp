...
std::list<btVector3> Path::createRandomPath(int   NumWaypoints,
                                            double MinX,
                                            double MinY,
                                            double MinZ,
                                            double MaxX,
...
//    gdi->OrangePen();
    ...
    {
        njli::World::getInstance()->getDebugDrawer()->drawLine(wp, *it, color);
        
        wp = *it++;
        njli::World::getInstance()->getDebugDrawer()->point(wp, color);
    }
    
    if (m_bLooped) njli::World::getInstance()->getDebugDrawer()->drawLine(*(--it), *m_WayPoints.begin(), color);
}
