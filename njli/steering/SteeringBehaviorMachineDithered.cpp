...
#define FORMATSTRING                                                           \
    "{\"njli::SteeringBehaviorMachineDithered\":[{\"name\":\"%s\"}]}"
#include "JsonJLI.h"
#include "btPrint.h"
...
namespace njli
{
    ...
    SteeringBehaviorMachineDithered &SteeringBehaviorMachineDithered::operator=(
        const SteeringBehaviorMachineDithered &rhs)
    {
        if (this != &rhs)
        {
        }
    ...
    void SteeringBehaviorMachineDithered::load(
        SteeringBehaviorMachineDithered &object, lua_State *L, int index)
    {
             ...
             iter != m_SteeringBehaviorVector.end(); ++iter)
        {
            ...
            if (RandFloat() < probability)
            {
                ...
                if (!force.isZero())
                {
                    ...
                    {
                        force =
                            force.normalized() * sb->getParent()->getMaxForce();
                    }
