...
static int s_dochunk(lua_State *L, int status)
{
    if (status == LUA_OK)
        status = s_docall(L, 0, 0);
    return report(L, status);
}

static int s_dofile(lua_State *L, const char *name)
{
    return s_dochunk(L, luaL_loadfile(L, name));
}

static int s_dostring(lua_State *L, const char *s, const char *name)
{
    return s_dochunk(L, luaL_loadbuffer(L, s, strlen(s), name));
}

// static int msghandler2(lua_State *L)
//{
//    const char *msg = lua_tostring(L, 1);
//    if (msg == NULL)
//    { /* is error object not a string? */
...
// ** (if present)
// */
// void njli::WorldLuaVirtualMachine::l_message(const char *pname, const char
// *msg)
//{
//  // if (pname)
//  //   lua_writestringerror("%s: ", pname);
...
static int appendLuaPath(lua_State *L, const char *path)
{
    ...
    lua_setfield(L, -2,
                 "path"); // set the field "path" in table at -2 with value at
                          // top of stack
...
LUALIB_API int luaopen_JLIM(lua_State *L)
{
    ...
    //    lua_pushvalue(L,-1);        // pluck these lines out if they offend
    //    you lua_setglobal(L,"JLIM"); // for they clobber the Holy _G
    return 1;
}
...
//    engine.\n\n"; char trigger_value[512]; char completion_value[512];
//
//    std::string trigger0 = "\t\t{\"trigger\": \"";
//    std::string trigger1 = "\", \"contents\": \"";
//    std::string trigger2 = "\" },\n";
...
static void printWrappedClasses(lua_State *L)
{
    ...
    //
    //    swig_module_info *module = SWIG_GetModule(L);
    //    swig_lua_class **bases = ((swig_lua_class
    //    *)(swig_type->clientdata))->bases; const char **base_names =
    ...
    //        {
    //            swig_lua_class*
    //            ptr=(swig_lua_class*)(swig_types[i]->clientdata);
...
namespace njli
{
    ...
    bool WorldLuaVirtualMachine::loadFile(const char *filePath)
    {
        ...
        //            else
        //            {
        //                SDL_LogWarn(SDL_LOG_CATEGORY_TEST, "Unable to load
        //                script file");
        //            }
        //        }
        //
    ...
    {
        return WorldLuaVirtualMachine::loadString(m_lua_State, code);
    }
    ...
    {
        return WorldLuaVirtualMachine::doString(m_lua_State, code);
    }
    ...
    void WorldLuaVirtualMachine::init()
    {
        ...
        no\n"); #endif
                ...
                SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", "Lua
        LUA_USE_LONGJMP: yes\n"); #else SDL_LogVerbose(SDL_LOG_CATEGORY_TEST,
        "%s", "Lua LUA_USE_LONGJMP: no\n"); #endif
        ...
        no\n");
                ...
                SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", "Lua
        LUA_USE_ULONGJMP: yes\n"); #else SDL_LogVerbose(SDL_LOG_CATEGORY_TEST,
        "%s", "Lua LUA_USE_ULONGJMP: no\n"); #endif*/
    ...
    bool WorldLuaVirtualMachine::execute(const char *code, s32 width,
