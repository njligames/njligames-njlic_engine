...
{
  LabelHUD::LabelHUD()
      : AbstractFactoryObject(this), m_fontSize(48), m_fontFace("sans"),
  ...
  LabelHUD::LabelHUD(const AbstractBuilder &builder)
      : AbstractFactoryObject(this), m_fontSize(128), m_fontFace("sans"),
        m_fillColor(new btVector4(1.0f, 0.0f, 0.0f, 1.0f)),
        m_align(JLI_HUD_ALIGN_LEFT | JLI_HUD_ALIGN_MIDDLE),
        m_position(new btVector2(0.0f, getHeight())),
        m_dimensions(new btVector2(280.0f, 20.0f)), m_text("EMPTY")
  {
    //        m_FBO = njli::World::getInstance()->getWorldHUD()->createFBO(512,
    //        512, NVG_IMAGE_REPEATX | NVG_IMAGE_REPEATY);
  }
  ...
  const char *LabelHUD::getClassName() const { return "LabelHUD"; }

  s32 LabelHUD::getType() const { return JLI_OBJECT_TYPE_LabelHUD; }
  ...
  {
    return (LabelHUD **)World::getInstance()->getWorldFactory()->createArray(
        LabelHUD::type(), size);
  }
  ...
  {
    return dynamic_cast<LabelHUD *>(
        World::getInstance()->getWorldFactory()->create(LabelHUD::type()));
  }
  ...
  u32 LabelHUD::type() { return JLI_OBJECT_TYPE_LabelHUD; }

  void LabelHUD::setFontSize(f32 size) { m_fontSize = size; }

  f32 LabelHUD::getFontSize() const { return m_fontSize; }

  void LabelHUD::setFontFace(const char *fontFace) { m_fontFace = fontFace; }

  const char *LabelHUD::getFontFace() const { return m_fontFace.c_str(); }

  void LabelHUD::setFillColor(const btVector4 &color) { *m_fillColor = color; }

  const btVector4 &LabelHUD::getFillColor() const { return *m_fillColor; }

  void LabelHUD::setAlign(s32 align) { m_align = align; }

  s32 LabelHUD::getAlign() const { return m_align; }

  void LabelHUD::setPosition(const btVector2 &pos) { *m_position = pos; }

  const btVector2 &LabelHUD::getPosition() const { return *m_position; }
