...
namespace njli
{
  ...
  void WorldDebugDrawer::editTransform(const Camera &camera,
                                       btTransform &matrix)
  {
    ...
    //        matrixRotation, matrixScale, matrix.m16);
    ...
    //        mCurrentGizmoOperation, mCurrentGizmoMode, matrix.m16, NULL,
    //        useSnap ? &snap.x : NULL);
  }
...
#undef OFFSETOF
    ...
    (GL_FALSE == depthFace) ? glDisable(GL_DEPTH_TEST)
                            : glEnable(GL_DEPTH_TEST);
    (GL_FALSE == scissorTest) ? glDisable(GL_SCISSOR_TEST)
                              : glEnable(GL_SCISSOR_TEST);
