...
namespace njli
{
  DropdownHUD::DropdownHUD() : AbstractFactoryObject(this) {}
  ...
  const char *DropdownHUD::getClassName() const { return "DropdownHUD"; }

  s32 DropdownHUD::getType() const { return JLI_OBJECT_TYPE_DropdownHUD; }
  ...
  {
    return (DropdownHUD **)World::getInstance()->getWorldFactory()->createArray(
        DropdownHUD::type(), size);
  }
  ...
  {
    return dynamic_cast<DropdownHUD *>(
        World::getInstance()->getWorldFactory()->create(DropdownHUD::type()));
  }
  ...
  u32 DropdownHUD::type() { return JLI_OBJECT_TYPE_DropdownHUD; }
}
