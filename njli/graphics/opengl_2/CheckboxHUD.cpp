...
namespace njli
{
  CheckboxHUD::CheckboxHUD()
      : AbstractFactoryObject(this),
        ...
        m_iconFontSize(40 * SCALE), m_iconFontFace("icons"),
        m_iconFillColor(new btVector4(1.0f, 1.0f, 1.0f, 128.0f / 255.0f)),
        m_iconText(
            njli::World::getInstance()->getWorldHUD()->cpToUTF8(ICON_CHECK)),
        m_text("CHECKBOX"),
  ...
  CheckboxHUD::~CheckboxHUD()
  {
    delete m_checkboxDimension;
    m_checkboxDimension = NULL;
    delete m_iconFillColor;
    m_iconFillColor = NULL;
  ...
  void CheckboxHUD::renderFunction(Camera *camera)
  {
    ...
    JLIPaint bg = hud->boxGradient(
        x + 1, y + (h * 0.5) - (m_checkboxDimension->y() * 0.5f) + 1,
        m_checkboxDimension->x(), m_checkboxDimension->y(), m_cornerRadius,
        m_feather, *m_gradientStartColor, *m_gradientEndColor);
    hud->beginPath();
    hud->roundedRect(x + 1, y + (h * 0.5) - (m_checkboxDimension->y() * 0.5f),
                     m_checkboxDimension->x(), m_checkboxDimension->y(),
                     m_cornerRadius);
    hud->fillPaint(bg);
    hud->fill();
    ...
    if (m_isChecked)
      {
        hud->textAlign((njliHUDAlign)m_align);
        hud->text(x + (m_checkboxDimension->x() * 0.3f), y + (h * 0.4f),
                  m_iconText.c_str());
      }
  ...
  const char *CheckboxHUD::getClassName() const { return "CheckboxHUD"; }

  s32 CheckboxHUD::getType() const { return JLI_OBJECT_TYPE_CheckboxHUD; }
  ...
  {
    return (CheckboxHUD **)World::getInstance()->getWorldFactory()->createArray(
        CheckboxHUD::type(), size);
  }
  ...
  {
    return dynamic_cast<CheckboxHUD *>(
        World::getInstance()->getWorldFactory()->create(CheckboxHUD::type()));
  }
  ...
  u32 CheckboxHUD::type() { return JLI_OBJECT_TYPE_CheckboxHUD; }

  void CheckboxHUD::setDimensions(const btVector2 &dim) { *m_dimensions = dim; }

  void CheckboxHUD::setPosition(const btVector2 &pos) { *m_position = pos; }

  void CheckboxHUD::setFontSize(f32 fontSize) { m_fontSize = fontSize; }

  void CheckboxHUD::setFontFace(const char *fontFace) { m_fontFace = fontFace; }
  ...
  void CheckboxHUD::setText(const char *text) { m_text = text; }

  void CheckboxHUD::setFontAlign(njliHUDAlign align) { m_align = align; }

  void CheckboxHUD::setCheckboxRadius(f32 radius) { m_cornerRadius = radius; }

  void CheckboxHUD::setCheckboxFeather(f32 feather) { m_feather = feather; }
  ...
  void CheckboxHUD::setFontSizeIcon(f32 size) { m_iconFontSize = size; }
  ...
  void CheckboxHUD::setTextIcon(const char *text) { m_iconText = text; }

  const btVector2 &CheckboxHUD::getDimensions() const { return *m_dimensions; }
  ...
  f32 CheckboxHUD::getFontSize() const { return m_fontSize; }

  const char *CheckboxHUD::getFontFace() const { return m_fontFace.c_str(); }

  const btVector4 &CheckboxHUD::getFontColor() const { return *m_fillColor; }

  const char *CheckboxHUD::getText() const { return m_text.c_str(); }
  ...
  f32 CheckboxHUD::getCheckboxRadius() const { return m_cornerRadius; }

  f32 CheckboxHUD::getCheckboxFeather() const { return m_feather; }
  ...
  f32 CheckboxHUD::getFontSizeIcon() const { return m_iconFontSize; }
  ...
  const char *CheckboxHUD::getTextIcon() const { return m_iconText.c_str(); }

  void CheckboxHUD::check() { m_isChecked = true; }
