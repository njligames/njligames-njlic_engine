...
namespace njli
{
  ...
  MaterialProperty::MaterialProperty()
      : AbstractFactoryObject(this),
        ...
        m_materialBound(new bool[16]), m_textureID(-1),
        ...
        m_TextureIndex(-1), m_Width(0), m_Height(0), m_NumberOfComponents(0),
        m_2DUniform(-1), m_CubeUniform(-1), m_is2DUniform(-1),
        m_isCubeUniform(-1), m_AbstractFrameBufferObject(NULL),
        m_hasOpacity(false), m_njliLoadGPUType(JLI_LOAD_GPU_TYPE_NONE),
        m_loadGPU_fbo(NULL) //,
  ...
  void MaterialProperty::serialize(void *dataBuffer,
  ...
  s32 MaterialProperty::getType() const { return MaterialProperty::type(); }
  ...
  {
    return (MaterialProperty **)World::getInstance()
        ->getWorldFactory()
        ->createArray(MaterialProperty::type(), size);
  }
  ...
  {
    return dynamic_cast<MaterialProperty *>(
        World::getInstance()->getWorldFactory()->create(
            MaterialProperty::type()));
  }

  MaterialProperty *
  MaterialProperty::create(const MaterialPropertyBuilder &builder)
  {
  ...
  u32 MaterialProperty::type() { return JLI_OBJECT_TYPE_MaterialProperty; }
  ...
  void
  MaterialProperty::setMinificationFilter(njliTextureMinificationValueType val)
  {
    SDL_assert(val > JLI_TEXTURE_MINIFICATION_VALUE_TYPE_NONE &&
               val < JLI_TEXTURE_MINIFICATION_VALUE_TYPE_MAX);
    m_minVal = val;
  }

  njliTextureMinificationValueType
  MaterialProperty::getMinificationFilter() const
  {
    return m_minVal;
  }

  void MaterialProperty::setMagnificationFilter(
      njliTextureMagnificationValueType val)
  {
    SDL_assert(val > JLI_TEXTURE_MAGNIFICATION_VALUE_TYPE_NONE &&
               val < JLI_TEXTURE_MAGNIFICATION_VALUE_TYPE_MAX);
    m_magVal = val;
  }

  njliTextureMagnificationValueType
  MaterialProperty::getMagnificationFilter() const
  {
    return m_magVal;
  }
  ...
  {
    SDL_assert(val > JLI_TEXTURE_WRAP_TYPE_NONE &&
               val < JLI_TEXTURE_WRAP_TYPE_MAX);
    m_sWrap = val;
  }

  njliTextureWrapType MaterialProperty::getWrapS() const { return m_sWrap; }
  ...
  njliTextureWrapType MaterialProperty::getWrapT() const { return m_tWrap; }
  ...
  void MaterialProperty::loadGPU(const Image &negativeX, const Image &negativeY,
                                 ...
                                 const Image &positiveY, const Image &positiveZ)
  {
    ...
    loadGPU_Internal(negativeX, negativeY, negativeZ, positiveX, positiveY,
                     positiveZ);
  }
  ...
  void MaterialProperty::reLoadGPU(const Image &img,
                                   njliTextureCubeSides cubeSide,
  ...
  u32 MaterialProperty::getWidth() const { return m_Width; }

  u32 MaterialProperty::getHeight() const { return m_Height; }
  ...
  bool MaterialProperty::hasOpacity() const { return m_hasOpacity; }

  u8 MaterialProperty::getTextureIndex() const { return m_TextureIndex; }
  ...
  void MaterialProperty::loadTexImage2DInternal(const Image &img, s32 target)
  {
    ...
    if (img.isCompressed())
      {
        glCompressedTexImage2D(target, level, internalformat, m_Width, m_Height,
                               border, imageSize, pixels);
        DEBUG_GL_ERROR_WRITE("glCompressedTexImage2D");
      }
    else
      {
        glTexImage2D(target, level, internalformat, m_Width, m_Height, border,
                     format, type, pixels);
  ...
  void MaterialProperty::reLoadTexImage2DInternal(const Image &img, s32 target,
                                                  const btVector2 &offset)
  {
    GLint level = 0;
    GLint internalformat = GL_RGBA;
    ...
    if (img.isCompressed())
      {
        glCompressedTexSubImage2D(target, level, offset.x(), offset.y(),
                                  m_Width, m_Height, format, imageSize, pixels);
  ...
  {
    SDL_assert(m_minVal > JLI_TEXTURE_MINIFICATION_VALUE_TYPE_NONE &&
               m_minVal < JLI_TEXTURE_MINIFICATION_VALUE_TYPE_MAX);
    SDL_assert(m_magVal > JLI_TEXTURE_MAGNIFICATION_VALUE_TYPE_NONE &&
               m_magVal < JLI_TEXTURE_MAGNIFICATION_VALUE_TYPE_MAX);
    SDL_assert(m_tWrap > JLI_TEXTURE_WRAP_TYPE_NONE &&
               m_tWrap < JLI_TEXTURE_WRAP_TYPE_MAX);
    SDL_assert(m_sWrap > JLI_TEXTURE_WRAP_TYPE_NONE &&
...
//    }
    ...
    if (img.isPvr())
      {
        ...
        //            {
        //                SDL_LogError(SDL_LOG_CATEGORY_TEST, "ERROR: Failed to
        //                load texture.");
        //            }
      }
    else
  ...
  void MaterialProperty::loadGPU_Internal(
      const Image &negativeX, const Image &negativeY, const Image &negativeZ,
      const Image &positiveX, const Image &positiveY, const Image &positiveZ)
  {
    ...
    m_hasOpacity = negativeX.hasAlpha() || negativeY.hasAlpha() ||
                   negativeZ.hasAlpha() || positiveX.hasAlpha() ||
                   positiveY.hasAlpha() || positiveZ.hasAlpha();
  }
