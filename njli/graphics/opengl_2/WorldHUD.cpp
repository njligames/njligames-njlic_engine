...
{
  WorldHUD::WorldHUD()
      : m_NVGContext(NULL), m_matrixBuffer(new f32[9]), m_Camera(NULL)
  {
    m_NVGContext =
  ...
  WorldHUD::~WorldHUD()
  {
    ...
    for (std::vector<s32>::iterator i = m_images.begin(); i != m_images.end();
         ++i)
  ...
  const s8 *WorldHUD::getClassName() const { return "WorldHUD"; }

  s32 WorldHUD::getType() const { return JLI_OBJECT_TYPE_WorldHUD; }
  ...
  void WorldHUD::transformTranslate(btMatrix3x3 &dst,
  ...
  void WorldHUD::transformPoint(btMatrix3x3 &dstx, btMatrix3x3 &dsty,
                                btMatrix3x3 &xform, const btVector2 &sourcePos)
  {
    ...
    WorldHUD::transformPoint(_dstx, _dsty, _xform, sourcePos.x(),
                             sourcePos.y());
  ...
  void WorldHUD::transformPoint(f32 *dstx, f32 *dsty, f32 *xform, f32 srcx,
                                f32 srcy)
  {
    nvgTransformPoint(dstx, dsty, xform, srcx, srcy);
  }

  f32 WorldHUD::degToRad(f32 deg) { return nvgDegToRad(deg); }

  f32 WorldHUD::radToDeg(f32 rad) { return nvgRadToDeg(rad); }

  void WorldHUD::save() { nvgSave(m_NVGContext); }

  void WorldHUD::restore() { nvgRestore(m_NVGContext); }

  void WorldHUD::reset() { nvgReset(m_NVGContext); }
  ...
  void WorldHUD::miterLimit(f32 limit) { nvgMiterLimit(m_NVGContext, limit); }

  void WorldHUD::strokeWidth(f32 size) { nvgStrokeWidth(m_NVGContext, size); }
  ...
  void WorldHUD::globalAlpha(f32 alpha) { nvgGlobalAlpha(m_NVGContext, alpha); }

  void WorldHUD::resetTransform() { nvgResetTransform(m_NVGContext); }
  ...
  void WorldHUD::transform(const btMatrix3x3 &mtx)
  {
    ...
    this->transform(m_matrixBuffer[0], m_matrixBuffer[3], m_matrixBuffer[1],
                    m_matrixBuffer[4], m_matrixBuffer[2], m_matrixBuffer[5]);
  }
  ...
  void WorldHUD::rotate(f32 angle) { nvgRotate(m_NVGContext, angle); }

  void WorldHUD::skewX(f32 angle) { nvgSkewX(m_NVGContext, angle); }

  void WorldHUD::skewY(f32 angle) { nvgSkewY(m_NVGContext, angle); }
  ...
  s32 WorldHUD::createImageRGBA(const Image &image,
                                njliHUDImageFlags imageFlags)
  {
    s32 img =
        nvgCreateImageRGBA(m_NVGContext, image.getWidth(), image.getHeight(),
                           imageFlags, image.getDataPtr());
  ...
  {
    m_images.erase(std::remove(m_images.begin(), m_images.end(), imageHandle),
                   m_images.end());
    nvgDeleteImage(m_NVGContext, imageHandle);
  }

  JLIPaint WorldHUD::linearGradient(const btVector2 &startPos,
                                    const btVector2 &endPos,
                                    const btVector4 &startColor,
                                    const btVector4 &endColor)
  {
    return this->linearGradient(startPos.x(), startPos.y(), endPos.x(),
                                endPos.y(), startColor, endColor);
  }

  JLIPaint WorldHUD::boxGradient(const Rect &rect, f32 radius, f32 feather,
                                 const btVector4 &startColor,
                                 const btVector4 &endColor)
  {
    return this->boxGradient(rect.getOffset().x(), rect.getOffset().y(),
                             rect.getDimension().x(), rect.getDimension().y(),
                             radius, feather, startColor, endColor);
  }

  JLIPaint WorldHUD::radialGradient(const btVector2 &center, f32 innerRadius,
                                    f32 outerRadius,
                                    const btVector4 &startColor,
                                    const btVector4 &endColor)
  {
    return this->radialGradient(center.x(), center.y(), innerRadius,
                                outerRadius, startColor, endColor);
  }

  JLIPaint WorldHUD::imagePattern(const btVector2 &startPos,
                                  const btVector2 &endPos, f32 angle,
                                  s32 imageHandle, f32 alpha)
  {
    return this->imagePattern(startPos.x(), startPos.y(), endPos.x(),
                              endPos.y(), angle, imageHandle, alpha);
  }

  void WorldHUD::scissor(const Rect &rect)
  {
    this->scissor(rect.getOffset().x(), rect.getOffset().y(),
                  rect.getDimension().x(), rect.getDimension().y());
  }

  void WorldHUD::intersectScissor(const Rect &rect)
  {
    this->intersectScissor(rect.getOffset().x(), rect.getOffset().y(),
                           rect.getDimension().x(), rect.getDimension().y());
  }

  void WorldHUD::resetScissor() { nvgResetScissor(m_NVGContext); }

  void WorldHUD::beginPath() { nvgBeginPath(m_NVGContext); }
  ...
  void WorldHUD::bezierTo(const btVector2 &controlPoint1,
                          const btVector2 &controlPoint2, const btVector2 &pos)
  {
    this->bezierTo(controlPoint1.x(), controlPoint1.y(), controlPoint2.x(),
                   controlPoint2.y(), pos.x(), pos.y());
  }
  ...
  void WorldHUD::closePath() { nvgClosePath(m_NVGContext); }
  ...
  void WorldHUD::arc(const btVector2 &center, f32 radius, f32 fromAngle,
                     f32 toAngle, njliHUDWinding dir)
  {
  ...
  {
    this->rect(rect.getOffset().x(), rect.getOffset().y(),
               rect.getDimension().x(), rect.getDimension().y());
  }

  void WorldHUD::roundedRect(const Rect &rect, f32 r)
  {
    this->roundedRect(rect.getOffset().x(), rect.getOffset().y(),
                      rect.getDimension().x(), rect.getDimension().y(), r);
  }
  ...
  void WorldHUD::fill() { nvgFill(m_NVGContext); }

  void WorldHUD::stroke() { nvgStroke(m_NVGContext); }
  ...
  //    s32 WorldHUD::createFontMem(const s8* name, u8* data, s32 ndata, s32
  //    freeData)
  //    {
  ////        nvgCreateFontMem(m_NVGContext, <#const s8 *name#>, <#u8 *data#>,
  ///<#s32 ndata#>, <#s32 freeData#>)
  //    }
  ...
  void WorldHUD::fontSize(f32 size) { nvgFontSize(m_NVGContext, size); }

  void WorldHUD::fontBlur(f32 blur) { nvgFontBlur(m_NVGContext, blur); }
  ...
  //    void WorldHUD::textAlign(njliHUDAlign align)
  //    {
  //        this->textAlign(align, JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE,
  //        JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE,
  //        JLI_HUD_ALIGN_NONE);
  //    }
  //
  //    void WorldHUD::textAlign(njliHUDAlign align1, njliHUDAlign align2)
  //    {
  //        this->textAlign(align1, align2, JLI_HUD_ALIGN_NONE,
  //        JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE,
  //        JLI_HUD_ALIGN_NONE);
  //    }
  //
  ...
  //    njliHUDAlign align3)
  //    {
  //        this->textAlign(align1, align2, align3, JLI_HUD_ALIGN_NONE,
  //        JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE);
  //    }
  //    void WorldHUD::textAlign(njliHUDAlign align1, njliHUDAlign align2,
  //    njliHUDAlign align3, njliHUDAlign align4)
  //    {
  //        this->textAlign(align1, align2, align3, align4, JLI_HUD_ALIGN_NONE,
  //        JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE);
  //    }
  //    void WorldHUD::textAlign(njliHUDAlign align1, njliHUDAlign align2,
  //    njliHUDAlign align3, njliHUDAlign align4, njliHUDAlign align5)
  //    {
  //        this->textAlign(align1, align2, align3, align4, align5,
  //        JLI_HUD_ALIGN_NONE, JLI_HUD_ALIGN_NONE);
  //    }
  //
  //    void WorldHUD::textAlign(njliHUDAlign align1, njliHUDAlign align2,
  ...
  //    njliHUDAlign align6)
  //    {
  //        this->textAlign(align1, align2, align3, align4, align5, align6,
  //        JLI_HUD_ALIGN_NONE);
  //    }
  //
  //    void WorldHUD::textAlign(njliHUDAlign align1, njliHUDAlign align2,
  //    njliHUDAlign align3, njliHUDAlign align4, njliHUDAlign align5,
  //    njliHUDAlign align6, njliHUDAlign align7)
  //    {
  //        nvgTextAlign(m_NVGContext, align1 | align2 | align3 | align4 |
  //        align5 | align6 | align7);
  //    }
  ...
  void WorldHUD::fontFace(const s8 *font) { nvgFontFace(m_NVGContext, font); }
  ...
  void WorldHUD::textBox(const btVector2 &position, f32 breakRowWidth,
                         const s8 *string, const s8 *end)
  {
    this->textBox(position.x(), position.y(), breakRowWidth, string, end);
  }

  f32 WorldHUD::textBounds(const btVector2 &position, Rect &bounds,
                           const s8 *string, const s8 *end)
  {
    return this->textBounds(position.x(), position.y(), bounds, string, end);
  }
  void WorldHUD::textBoxBounds(const btVector2 &position, f32 breakRowWidth,
                               Rect &bounds, const s8 *string, const s8 *end)
  {
    this->textBoxBounds(position.x(), position.y(), breakRowWidth, bounds,
                        string, end);
  }

  void WorldHUD::textGlyphPositions(
      const btVector2 &position, const s8 *string, const s8 *end,
      btAlignedObjectArray<JLIGlyphPosition> &glyphPositions)
  {
    this->textGlyphPositions(position.x(), position.y(), string, end,
                             glyphPositions);
  }

  void WorldHUD::textMetrics(btMatrix3x3 &ascender, btMatrix3x3 &descender,
                             btMatrix3x3 &lineh)
  {
  ...
  WorldHUD::textBreakLines(const s8 *string, const s8 *end, f32 breakRowWidth)
  {
    btAlignedObjectArray<JLITextRow> textRows;
    NVGtextRow *positions = new NVGtextRow[100];

    s32 numRows = nvgTextBreakLines(m_NVGContext, string, end, breakRowWidth,
                                    positions, 100);
  ...
  void WorldHUD::transform(f32 scaleX, f32 skewY, f32 skewX, f32 scaleY, f32 x,
                           f32 y)
  {
    nvgTransform(m_NVGContext, scaleX, skewY, skewX, scaleY, x, y);
  }

  void WorldHUD::translate(f32 x, f32 y) { nvgTranslate(m_NVGContext, x, y); }
  ...
  JLIPaint WorldHUD::linearGradient(f32 startx, f32 starty, f32 endx, f32 endy,
                                    const btVector4 &startColor,
                                    const btVector4 &endColor)
  {
    NVGcolor icol, ocol;
    ...
    JLIPaint ret;
    NVGPaintToJLIPaint(ret, nvgLinearGradient(m_NVGContext, startx, starty,
                                              endx, endy, icol, ocol));
    return ret;
  }

  JLIPaint WorldHUD::boxGradient(f32 x, f32 y, f32 width, f32 height,
                                 f32 radius, f32 feather,
                                 const btVector4 &startColor,
                                 const btVector4 &endColor)
  {
    NVGcolor icol, ocol;
    ...
    JLIPaint ret;
    NVGPaintToJLIPaint(ret, nvgBoxGradient(m_NVGContext, x, y, width, height,
                                           radius, feather, icol, ocol));
    return ret;
  }

  JLIPaint WorldHUD::radialGradient(f32 centerx, f32 centery, f32 innerRadius,
                                    f32 outerRadius,
                                    const btVector4 &startColor,
                                    const btVector4 &endColor)
  {
    ...
    JLIPaint ret;
    NVGPaintToJLIPaint(ret,
                       nvgRadialGradient(m_NVGContext, centerx, centery,
                                         innerRadius, outerRadius, icol, ocol));
    return ret;
  }

  JLIPaint WorldHUD::imagePattern(f32 startx, f32 starty, f32 endx, f32 endy,
                                  f32 angle, s32 imageHandle, f32 alpha)
  {
    JLIPaint ret;
    NVGPaintToJLIPaint(ret, nvgImagePattern(m_NVGContext, startx, starty, endx,
                                            endy, angle, imageHandle, alpha));
    return ret;
  }
  ...
  void WorldHUD::moveTo(f32 x, f32 y) { nvgMoveTo(m_NVGContext, x, y); }

  void WorldHUD::lineTo(f32 x, f32 y) { nvgLineTo(m_NVGContext, x, y); }

  void WorldHUD::bezierTo(f32 controlPoint1x, f32 controlPoint1y,
                          f32 controlPoint2x, f32 controlPoint2y, f32 x, f32 y)
  {
    nvgBezierTo(m_NVGContext, controlPoint1x, controlPoint1y, controlPoint2x,
                controlPoint2y, x, y);
  }
  ...
  void WorldHUD::arc(f32 centerx, f32 centery, f32 radius, f32 fromAngle,
                     f32 toAngle, njliHUDWinding dir)
  {
  ...
  void WorldHUD::textBox(f32 x, f32 y, f32 breakRowWidth, const s8 *string,
                         const s8 *end)
  {
    if (!string)
      string = "<NULL>";
  ...
  f32 WorldHUD::textBounds(f32 x, f32 y, Rect &bounds, const s8 *string,
                           const s8 *end)
  {
  ...
  void WorldHUD::textBoxBounds(f32 x, f32 y, f32 breakRowWidth, Rect &bounds,
                               const s8 *string, const s8 *end)
  {
    f32 *_bounds = new f32[4];
  ...
  //    f32 WorldHUD::textBounds(f32 x, f32 y, const s8* string, const s8* end,
  //    const btMatrix3x3 &bounds)
  //    {
  //        if(!string)
  //            string = "<NULL>";
  //        btMatrix3x3ToNVGaffine(m_matrixBuffer, bounds);
  //        return nvgTextBounds(m_NVGContext, x, y, string, end,
  //        m_matrixBuffer);
  //    }
  //
  //    void WorldHUD::textBoxBounds(f32 x, f32 y, f32 breakRowWidth, const s8*
  //    string, const s8* end, const btMatrix3x3 & bounds)
  //    {
  //        if(!string)
  //            string = "<NULL>";
  //        btMatrix3x3ToNVGaffine(m_matrixBuffer, bounds);
  //        nvgTextBoxBounds(m_NVGContext, x, y, breakRowWidth, string, end,
  //        m_matrixBuffer);
  //    }

  void WorldHUD::textGlyphPositions(
      f32 x, f32 y, const s8 *string, const s8 *end,
      btAlignedObjectArray<JLIGlyphPosition> &glyphPositions)
  {
    NVGglyphPosition *positions = new NVGglyphPosition[100];

    s32 numGlyphs =
        nvgTextGlyphPositions(m_NVGContext, x, y, string, end, positions, 100);
  ...
  //    {
  //        std::vector<NVGLUframebuffer*>::iterator iter =
  //        std::find(m_NVGLUfb.begin(), m_NVGLUfb.end(), fb);
  ...
  void WorldHUD::JLIPaintToNVGPaint(NVGpaint &nvgPaint,
  ...
  void WorldHUD::NVGPaintToJLIPaint(JLIPaint &njliPaint,
  ...
  void WorldHUD::JLIGlyphPositionToNVGglyphPosition(
      NVGglyphPosition &nvgGlyphPos, const JLIGlyphPosition &njliGlyphPos)
  {
  ...
  void WorldHUD::NVGglyphPositionToJLIGlyphPosition(
      JLIGlyphPosition &njliGlyphPos, const NVGglyphPosition &nvgGlyphPos)
  {
  ...
  void WorldHUD::JLITextRowToNVGtextRow(NVGtextRow &nvgTextRow,
  ...
  void WorldHUD::NVGtextRowToJLITextRow(JLITextRow &njliTextRow,
  ...
  {
    mtx = btMatrix3x3(floats[0], floats[1], floats[2], floats[3], floats[4],
    ...
    if (m_FBOvector.size() > 0)
      {
        ...
        for (std::vector<AbstractFrameBufferObject *>::iterator i =
                 m_FBOvector.begin();
             i != m_FBOvector.end(); ++i)
          {
  ...
  NVGcontext *WorldHUD::getContext() { return m_NVGContext; }

  void WorldHUD::addFBO(AbstractFrameBufferObject *fbo)
  {
    std::vector<AbstractFrameBufferObject *>::iterator iter =
        std::find(m_FBOvector.begin(), m_FBOvector.end(), fbo);
