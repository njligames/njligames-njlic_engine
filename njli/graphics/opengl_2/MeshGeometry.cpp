...
namespace njli
{
  ...
  MeshGeometry &MeshGeometry::operator=(const MeshGeometry &) { return *this; }
  ...
  void MeshGeometry::serialize(void *dataBuffer, btSerializer *serializer) const
  {
    // TODO: serialize
  }
  ...
  MeshGeometry *MeshGeometry::create()
  {
    return dynamic_cast<MeshGeometry *>(
        World::getInstance()->getWorldFactory()->create(MeshGeometry::type()));
  }
    ...
    if (m_TotalSubdivisions < maxNumberOfSubDivisions())
      {
        ...
        for (GLsizei currentIndice = 0, newIndice = 0;
             ...
             currentIndice += 3, newIndice += 12)
          {
            subdivideTriangle(m_VertexData[currentIndice + 0],
                              ...
                              m_VertexData[currentIndice + 2], m_triangleBuffer,
                              newIndice, m_indiceBuffer);
        ...
        for (unsigned long meshIndex = 0; meshIndex < maxNumberOfInstances();
             meshIndex++)
          {
            ...
            //                       m_IndiceDataBuffer + (GLuint)((meshIndex *
                                            ...
                                            const GLsizei verticeIdx) const
  {
    ...
    if (instanceIdx < maxNumberOfInstances() && verticeIdx < numberOfVertices())
      {
  ...
  TexturedColoredVertex MeshGeometry::getVertex(const GLsizei instanceIdx,
                                                const GLsizei verticeIdx) const
  {
  ...
  void MeshGeometry::loadData()
  {
    ...
    for (std::vector<std::string>::iterator i = faces.begin(); i != faces.end();
         i++, idx++)
      {
    ...
    m_VertexData =
        new TexturedColoredVertex[numberOfVertices() * maxNumberOfInstances() *
    ...
    m_IndiceDataBuffer = new GLuint[numberOfIndices() * maxNumberOfInstances() *
                                    subdivisionBufferSize()];
    ...
    for (unsigned long meshIndex = 0; meshIndex < maxNumberOfInstances();
         meshIndex++)
      {
        for (unsigned long verticeIndex = 0; verticeIndex < numberOfVertices();
             verticeIndex++)
          {
    ...
    if (index >= 0 && m_VertexData)
      {
        ...
        unsigned long offset = index * numberOfVertices();
        for (unsigned long vertexIndex = 0; vertexIndex < numberOfVertices();
             vertexIndex++)
          {
  ...
  void MeshGeometry::applyShape(Node *node, PhysicsShape *physicsShape)
  {
    ...
    //        else if (strcmp(physicsShape->getClassName(), "PhysicsShapeBox")
    //        == 0)
    //        {
  ...
  void MeshGeometry::transformVertices(Node *node, const btTransform &transfrom)
  {
    assert(false && "todo");
  }
  ...
  GLsizei MeshGeometry::numberOfVertices() const { return m_NumberOfVertices; }
