...
namespace njli
{
    ...
    const char *MeshGeometry::getClassName() const
    {
        return "MeshGeometry";
    }
    ...
    MeshGeometry *
    MeshGeometry::create(const MeshGeometryBuilder &builder)
    {
    ...
    void MeshGeometry::load(ShaderProgram *shader, const std::string &filedata, unsigned int numInstances, unsigned int numSubDivisions)
    {
        m_TotalSubdivisions = 0;
        m_Filedata = filedata;
...
//        }
    ...
    void MeshGeometry::transformVertexColors(Node * node, const btTransform &transform)
    {
        
        assert(false && "todo");
    }
    
    void MeshGeometry::transformTextureCoordinates(Node * node, const btTransform &transform)
    {
