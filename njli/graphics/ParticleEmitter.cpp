...
namespace njli
{
    ParticleEmitter::ParticleEmitter()
        : AbstractFactoryObject(this), emitterType(0),
          ...
          duration(-1), blendFuncSource(0), blendFuncDestination(0),
          maxRadius(0), maxRadiusVariance(72.63f), minRadius(0),
          minRadiusVariance(0), rotatePerSecond(0), rotationStart(0),
          rotationStartVariance(153.95f), rotationEnd(0),
          rotationEndVariance(0), rotatePerSecondVariance(153.95f),
          emissionRate(0), emitCounter(0), particleCount(0), elapsedTime(0),
          active(true), particleIndex(0), m_Nodes(NULL), m_ParticleArray(NULL),
          m_particlesRootNode(NULL), m_geometry(NULL), m_body(NULL),
    ...
    bool ParticleEmitter::addParticle()
    {
        ...
        // Init the position of the particle.  This is based on the source
        // position of the particle emitter plus a configured variance.  The
        // RANDOM_MINUS_1_TO_1 macro allows the number to be both positive and
        // negative
        ...
        // Init the direction of the particle.  The newAngle is calculated using
        // the angle passed in and the angle variance.
        ...
        *(particle->direction) = vector * vectorSpeed;
        ...
        // Calculate the particles life span using the life span and variance
        // passed in
        particle->timeToLive =
        ...
        particle->radiusDelta =
            (endRadius - startRadius) / particle->timeToLive;
        particle->angle =
            btRadians(angle + angleVariance * RANDOM_MINUS_1_TO_1());
        ...
        // Calculate the color the particle should be when its life is over.
        // This is done the same way as the start color above
        ...
        // Calculate the delta which is to be applied to the particles color
        // during each cycle of its life.  The delta calculation uses the life
        // span of the particle to make sure that the particles color will
        // transition from the start to end color during its life time.  As the
        // game loop is using a fixed delta value we can calculate the delta
        // color once saving cycles in the update method

        *(particle->color) = start;
        particle->deltaColor->setX((end.x() - start.x()) /
                                   particle->timeToLive);
        particle->deltaColor->setY((end.y() - start.y()) /
                                   particle->timeToLive);
        particle->deltaColor->setZ((end.z() - start.z()) /
                                   particle->timeToLive);
        particle->deltaColor->setW((end.w() - start.w()) /
                                   particle->timeToLive);

        // Calculate the rotation
        f32 startA = rotationStart +
                     rotationStartVariance * RANDOM_MINUS_1_TO_1() * scale;
        f32 endA =
