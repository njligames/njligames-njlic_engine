...
namespace njli
{
    Sprite2D::Sprite2D() : m_SpritePivots(NULL), m_changedDimensionArray(NULL)
    {
    }

    Sprite2D::Sprite2D(const AbstractBuilder &)
        : m_SpritePivots(NULL), m_changedDimensionArray(NULL)
    {
    }
    ...
    Sprite2D &Sprite2D::operator=(const Sprite2D &) { return *this; }

    //  Sprite2D::Sprite2D()
    //      : Geometry(), m_Sprite2D(NULL), m_Indexes(NULL),
    //      m_SpritePivots(NULL)
    //  {
    ...
    //      : Geometry(copy), m_Sprite2D(NULL), m_Indexes(NULL),
    //      m_SpritePivots(NULL)
    //  {
    //    m_SpritePivots = new btVector2[getMaxMeshes()];
    //    m_changedDimensionArray = new bool[getMaxMeshes()];
    ...
    {
        return (Sprite2D **)World::getInstance()
            ->getWorldFactory()
            ->createArray(Sprite2D::type(), size);
    }
    ...
    void Sprite2D::load(ShaderProgram *shader, unsigned int numInstances,
...
f 2/1/1 4/4/1 3/2/1
    ...
    //  void Sprite2D::getVertices(LevelOfDetail *geometry, btVector3
    //  **vertices,
    //                             Node *node) const
    //  {
    //  }
    //
    //  u64 Sprite2D::getNumberOfVertices(LevelOfDetail *geometry, Node *node)
    //  const
    //  {
    //    return 0;
    //  }
    ...
    //  }
    //
    //  void Sprite2D::transformVertices(Node *node, const btTransform
    //  &transfrom)
    //  {
    ...
    //  }
    //
    //  void Sprite2D::transformVertexColors(Node *node, const btTransform
    //  &transform)
    //  {
    //  }
    //
    ...
    void Sprite2D::setSpriteAtlasFrame(Node *node, const f32 &xoffset,
                                       ...
                                       const f32 &ydim)
    {
        setTextureOffsets(node, btVector2(xoffset, yoffset),
                          btVector2(xdim, ydim));
    }
    ...
    //
    //    // Set up texture coordinates for all particles as these will not
    //    change. for (int i = 0; i < getMaxMeshes(); i++)
    //      {
    ...
    //  {
    //    // TODO: implement
    //    btVector2 bottomLeft(position.x() - halfSize, position.y() -
    //    halfSize); btVector2 bottomRight(position.x() + halfSize, position.y()
    //    - halfSize); btVector2 topLeft(position.x() - halfSize, position.y() +
    //    halfSize); btVector2 topRight(position.x() + halfSize, position.y() +
    //    halfSize);
    //
