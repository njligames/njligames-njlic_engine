...
void WrapperOgg::LoadFromMemory(char *data, int dataSize, SoundInfo *soundInfo)
{
  ...
  int ret =
      ov_open_callbacks((void *)&this->t, this->ov, NULL, -1, callbacks_ogg);
...
void WrapperOgg::DecompressStream(std::vector<char> &decompressBuffer,
                                  bool inLoop)
{
  decompressBuffer.clear();
  ...
  do
    {
      do
        {
          // Read up to a buffer's worth of decoded sound data
          bytes = ov_read(this->ov, this->bufArray, OGG_BUFFER_SIZE, endian, 2,
                          1, &bitStream);
