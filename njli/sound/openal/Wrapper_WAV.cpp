  ...
  if (this->minProcesssLengthAtOnce == -1)
  ...
  else
    {
      if (this->minProcesssLengthAtOnce % WAV_BUFFER_SIZE != 0)
        {
          SDL_Log(
              "OGG buffer size and OpenAL buffer size are not modulable !!");
...
void WrapperWav::LoadFromMemory(char *data, int dataSize, SoundInfo *soundInfo)
{
  ...
  this->t.curPtr = this->t.filePtr =
      data; // +sizeof(WAV_DESC) + sizeof(WAV_FORMAT));
...
float WrapperWav::GetTime() const
{
  ...
  return pos / (this->format.sampleRate * this->format.channels *
                this->format.bitsPerSample / 8);
}

float WrapperWav::GetTotalTime() const
{
  float pos = this->t.fileSize - sizeof(WAV_DESC) - sizeof(WAV_FORMAT) -
              sizeof(WAV_CHUNK);

  float t = pos / (this->format.sampleRate * this->format.channels *
                   this->format.bitsPerSample / 8);
...
void WrapperWav::DecompressStream(std::vector<char> &decompressBuffer,
                                  bool inLoop)
{
  decompressBuffer.clear();
