...
namespace njli
{
    ...
    PhysicsShapeStaticPlane **
    PhysicsShapeStaticPlane::createArray(const u32 size)
    {
    ...
    {
        *m_btStaticPlaneShape =
            btStaticPlaneShape(getPlaneNormal(), planeConstant);
    }

    void
    PhysicsShapeStaticPlane::calculateLocalInertia(btScalar mass,
    ...
    void
    PhysicsShapeStaticPlane::setCollisionShape(const btCollisionShape &shape)
    {
        SDL_assert(shape.getShapeType() ==
                   m_btStaticPlaneShape->getShapeType());
        const btStaticPlaneShape *staticplane =
            dynamic_cast<const btStaticPlaneShape *>(&shape);
        *m_btStaticPlaneShape = btStaticPlaneShape(
            staticplane->getPlaneNormal(), staticplane->getPlaneConstant());
    }
} // namespace njli
