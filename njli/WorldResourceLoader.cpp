...
namespace njli
{
    ...
    //    WorldResourceLoader::FileData::FileData(const void* buffer, long
    //    size): m_buffer(malloc(size)), m_fileSize(size), m_fileName("")
    //    {
    ...
    //
    //        //Check for a compressed format (The first 8 bytes will be 0, so
    //        the whole thing will be equal to the last 32 bits). if
    //        (PixelFormatPartHigh==0)
    //        {
    ...
    //    bool WorldResourceLoader::setPvrImage(const char *filePath, Image
    //    &img)
    //    {
    //        const void *content;
...
#define ROUND(val, places) (roundf(val * pow(10, places)) / (pow(10, places)))
    ...
    */
    ...
    /*
    bool WorldResourceLoader::loadZip(const char *filePath, const char
    *password)
    {
    ...
    //                            bool exists = getFileData(name, &p, &_size);
                            ...
                            {
                                addFileData(name);
                                SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "Added
    file: %s - size: %lu\n", name, _size);
                            }
                            else
                            {
