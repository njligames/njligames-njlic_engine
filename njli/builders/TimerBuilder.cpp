...
namespace njli
{
  ...
  s32 TimerBuilder::calculateSerializeBufferSize() const { return 0; }

  void TimerBuilder::serialize(void *dataBuffer, btSerializer *serializer) const
  {
  }

  u32 TimerBuilder::getObjectType() const { return JLI_OBJECT_TYPE_Timer; }

  const char *TimerBuilder::getClassName() const { return "TimerBuilder"; }

  s32 TimerBuilder::getType() const { return TimerBuilder::type(); }
  ...
  {
    return (TimerBuilder **)World::getInstance()
        ->getWorldFactory()
        ->createArray(TimerBuilder::type(), size);
  }
  ...
  {
    return dynamic_cast<TimerBuilder *>(
        World::getInstance()->getWorldFactory()->create(TimerBuilder::type()));
  }
