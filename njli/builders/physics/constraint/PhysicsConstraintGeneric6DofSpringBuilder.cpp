...
namespace njli
{
    ...
    s32
    PhysicsConstraintGeneric6DofSpringBuilder::calculateSerializeBufferSize()
        const
    {
        return 0;
    }
    ...
    PhysicsConstraintGeneric6DofSpringBuilder::createArray(const u32 size)
    {
        return (PhysicsConstraintGeneric6DofSpringBuilder **)
            World::getInstance()
                ->getWorldFactory()
                ->createArray(PhysicsConstraintGeneric6DofSpringBuilder::type(),
                              size);
    }
        ...
        while (lua_next(L, -2))
            {
                ...
                if (lua_istable(L, -2))
                    {
                        PhysicsConstraintGeneric6DofSpringBuilder::load(object,
                                                                        L, -2);
                    }
                else
                    {
