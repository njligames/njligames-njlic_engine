...
namespace njli
{
    ...
    s32
    SteeringBehaviorMachinePrioritizedBuilder::calculateSerializeBufferSize()
        const
    {
        return 0;
    }
    ...
    SteeringBehaviorMachinePrioritizedBuilder::createArray(const u32 size)
    {
        return (SteeringBehaviorMachinePrioritizedBuilder **)
            World::getInstance()
                ->getWorldFactory()
                ->createArray(SteeringBehaviorMachinePrioritizedBuilder::type(),
                              size);
    }
        ...
        while (lua_next(L, -2))
            {
                ...
                if (lua_istable(L, -2))
                    {
                        SteeringBehaviorMachinePrioritizedBuilder::load(object,
                                                                        L, -2);
                    }
                else
                    {
