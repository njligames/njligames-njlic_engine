...
namespace njli
{
    ...
    SteeringBehaviorObstacleAvoidanceBuilder::createArray(const u32 size)
    {
        return (SteeringBehaviorObstacleAvoidanceBuilder **)World::getInstance()
            ->getWorldFactory()
            ->createArray(SteeringBehaviorObstacleAvoidanceBuilder::type(),
                          size);
    }
    ...
    void SteeringBehaviorObstacleAvoidanceBuilder::load(
        SteeringBehaviorObstacleAvoidanceBuilder &object, lua_State *L,
        int index)
    {
        ...
        while (lua_next(L, -2))
            {
                ...
                if (lua_istable(L, -2))
                    {
                        SteeringBehaviorObstacleAvoidanceBuilder::load(object,
                                                                       L, -2);
                    }
                else
                    {
