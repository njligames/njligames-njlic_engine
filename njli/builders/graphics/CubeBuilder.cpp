...
namespace njli
{
  ...
  s32 CubeBuilder::calculateSerializeBufferSize() const { return 0; }

  void CubeBuilder::serialize(void *dataBuffer, btSerializer *serializer) const
  {
  }

  u32 CubeBuilder::getObjectType() const { return JLI_OBJECT_TYPE_Cube; }

  const char *CubeBuilder::getClassName() const { return "CubeBuilder"; }

  s32 CubeBuilder::getType() const { return CubeBuilder::type(); }
  ...
  {
    return (CubeBuilder **)World::getInstance()->getWorldFactory()->createArray(
        CubeBuilder::type(), size);
  }
  ...
  {
    return dynamic_cast<CubeBuilder *>(
        World::getInstance()->getWorldFactory()->create(CubeBuilder::type()));
  }
