...
#include "World.h"
#define FORMATSTRING "{\"njli::MeshGeometryBuilder\":[]}"
#include "JsonJLI.h"
#include "btPrint.h"
...
namespace njli
{
  ...
  MeshGeometryBuilder::MeshGeometryBuilder(const MeshGeometryBuilder &copy) {}
  ...
  void MeshGeometryBuilder::destroy(MeshGeometryBuilder *object)
  {
    if (object)
      {
    ...
    while (lua_next(L, -2))
      {
        ...
        if (lua_istable(L, -2))
          {
            MeshGeometryBuilder::load(object, L, -2);
          }
        else
          {
