                ...
                float *projection, int *viewport, float *windowCoordinate)
{
    ...
    float fTempo[8];
    // Modelview transform
    fTempo[0] = modelview[0] * objx + modelview[4] * objy +
                modelview[8] * objz + modelview[12]; // w is always 1
    fTempo[1] = modelview[1] * objx + modelview[5] * objy +
                modelview[9] * objz + modelview[13];
    fTempo[2] = modelview[2] * objx + modelview[6] * objy +
                modelview[10] * objz + modelview[14];
    fTempo[3] = modelview[3] * objx + modelview[7] * objy +
                modelview[11] * objz + modelview[15];
    // Projection transform, the final row of projection matrix is always [0 0
    // -1 0] so we optimize for that.
...
namespace njli
{
    struct OcclusionBuffer
    {
        ...
        inline bool draw(const btVector4 &a, const btVector4 &b,
                         const btVector4 &c, const btScalar minarea)
        {
    ...
    class SceneRenderer : public btDbvt::ICollide
    {
      ...
      public:
        ...
        virtual SIMD_FORCE_INLINE const OcclusionBuffer *
        getOcclusionBuffer() const
        {
            btAssert(m_ocb);
            return m_ocb;
        }
    ...
    {
        if (m_ocb)
            return (m_ocb->queryOccluder(node->volume.Center(),
                                         node->volume.Extents()));
        return false;
    }
    ...
    void SceneRenderer::render()
    {
        //        SDL_assert(m_Camera);
        //
        //        m_drawn = m_TranslucentEntities.size() +
        //        m_OpaqueEntities.size();
        //
    ...
    {
        render(pbp->m_sets[1].m_root, pbp->m_sets[0].m_root,
               getForwardVector());
    }
